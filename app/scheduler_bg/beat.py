...
from tasks import (
    ...
    health_check_task,
    scan_rss_for_stock_opportunities_task
)
...
celery_app.conf.beat_schedule = {
    ...
    # 🔍 INTELLIGENT STOCK DISCOVERY - Scan RSS articles for sector-specific opportunities
    # Run every 15 minutes to discover high-potential stocks before mainstream news
    'scan-stock-opportunities': {
        'task': 'sched.tasks.scan_rss_for_stock_opportunities_task',
        'schedule': crontab(minute='*/15'),  # Every 15 minutes
        'options': {
            'expires': 900,  # 15 minute expiry
            'retry_policy': {
...
celery_app.conf.update(
    ...
    task_routes={
        ...
        'sched.tasks.scan_rss_for_stock_opportunities_task': {'queue': 'scanner'},
